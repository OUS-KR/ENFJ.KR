...
let gameState = {
    ...
    villagers: [
        { id: "ella", name: "엘라", personality: "낙천적", skill: "농업", temperament: "온화함", trust: 70, status: "평온" },
        { id: "kai", name: "카이", personality: "현실적", skill: "벌목", temperament: "성실함", trust: 60, status: "평온" }
    ],
    maxVillagers: 5,
...
const gameScenarios = {
    ...
    "action_build_facility": {
        ...
        choices: [
            ...
            { text: "도서관 (나무 80, 돌 40 필요)", action: "build_library" },
...
const emotionMatchQuestions = [
    { scenario: "마을 주민이 오랫동안 기다리던 수확물이 예상치 못하게 흉작이 되었습니다. 주민은 깊은 한숨을 쉬며 고개를 떨굽니다.", choices: [{ text: "슬픔", isCorrect: true }, { text: "기쁨", isCorrect: false }, { text: "분노", isCorrect: false }, { text: "놀람", isCorrect: false }], feedback: "정답입니다! 주민의 실망과 좌절을 잘 이해하셨군요. (+5 공감)" },
    { scenario: "마을의 오랜 숙원 사업이었던 다리가 성공적으로 완공되었습니다. 주민들은 환호하며 서로를 얼싸안습니다.", choices: [{ text: "슬픔", isCorrect: false }, { text: "기쁨", isCorrect: true }, { text: "두려움", isCorrect: false }, { text: "혐오", isCorrect: false }], feedback: "정답입니다! 주민들의 성취감과 행복을 함께 느끼셨군요. (+5 행복도)" },
    { scenario: "밤늦게 마을 외곽에서 수상한 그림자가 목격되었다는 소식이 전해집니다. 주민들은 삼삼오오 모여 불안한 표정으로 속삭입니다.", choices: [{ text: "기쁨", isCorrect: false }, { text: "분노", isCorrect: false }, { text: "두려움", isCorrect: true }, { text: "놀람", isCorrect: false }], feedback: "정답입니다! 주민들의 불안감을 정확히 파악하셨군요. (+5 공동체 정신)" },
    { scenario: "마을 축제 준비 중, 한 주민이 다른 주민의 소중한 물건을 실수로 파손했습니다. 물건의 주인은 얼굴을 붉히며 거친 숨을 내쉽니다.", choices: [{ text: "슬픔", isCorrect: false }, { text: "분노", isCorrect: true }, { text: "놀람", isCorrect: false }, { text: "기쁨", isCorrect: false }], feedback: "정답입니다! 주민의 상실감과 화를 잘 헤아리셨군요. (+5 공감)" }
];
...
function setupEmotionMatchGame() {
    ...
    gameArea.innerHTML = `<p><strong>상황:</strong> ${currentEmotionQuestion.scenario}</p><p>이 상황에서 주민이 느낄 가장 적절한 감정은 무엇일까요?</p>`;
    currentEmotionQuestion.choices.forEach(choice => {
...
function checkEmotionMatchAnswer(isCorrect, feedbackMessage) {
    if (isCorrect) {
        ...
        updateState({ empathy: gameState.empathy + 5, happiness: gameState.happiness + 2 });
    } else {
...
function generateRandomVillager() {
    ...
    return { id, name, personality, skill, temperament: "평온", preferredAction: "none", trust: 50, status: "평온" };
}
...
const gameActions = {
    ...
    handle_conflict: (params) => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        const { first } = params;
        updateGameDisplay(`${first}의 이야기를 먼저 들어주었습니다. 그녀는 당신의 공감에 감사해합니다. (+10 공감, +5 행복도)`);
    ...
    ignore_event: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("갈등을 신경 쓰지 않았습니다. 엘라와 카이의 관계가 더 악화된 것 같습니다. (-15 행복도, -10 공동체 정신)");
        updateState({ happiness: gameState.happiness - 15, communitySpirit: gameState.communitySpirit - 10 });
        renderChoices(gameScenarios["intro"].choices);
    },
    distribute_food: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.food >= 5) { updateGameDisplay("개인 식량을 풀어 주민들에게 나눠주었습니다. (+15 행복도, -5 식량)"); updateState({ happiness: gameState.happiness + 15, resources: { ...gameState.resources, food: gameState.resources.food - 5 } }); }
        else { updateGameDisplay("식량이 부족하여 나눠줄 수 없었습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    request_frugality: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("주민들에게 식량 절약을 요청했습니다. (+5 공동체 정신, -5 행복도)");
        updateState({ communitySpirit: gameState.communitySpirit + 5, happiness: gameState.happiness - 5 });
        renderChoices(gameScenarios["intro"].choices);
    },
    seek_food_outside: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (currentRandFn() < 0.5) { updateGameDisplay("외부에서 식량을 구하는 데 성공했습니다! (+10 식량, +5 행복도)"); updateState({ resources: { ...gameState.resources, food: gameState.resources.food + 10 }, happiness: gameState.happiness + 5 }); }
        else { updateGameDisplay("외부에서 식량을 구하는 데 실패했습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    search_new_food_source: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (currentRandFn() < 0.7) { updateGameDisplay("새로운 식량원을 찾아냈습니다! (+10 식량, +10 공동체 정신)"); updateState({ resources: { ...gameState.resources, food: gameState.resources.food + 10 }, communitySpirit: gameState.communitySpirit + 10 }); }
        else { updateGameDisplay("새로운 식량원을 찾지 못했습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    welcome_new_unique_villager: () => {
        ...
        if (newVillager) {
            ...
            updateGameDisplay(`새로운 주민 ${newVillager.name}${getEulReParticle(newVillager.name)} 따뜻하게 환영했습니다. (+10 공동체 정신, +5 행복도)`);
            updateState({ communitySpirit: gameState.communitySpirit + 10, happiness: gameState.happiness + 5, villagers: gameState.villagers });
            delete gameState.pendingNewVillager;
        }
        renderChoices(gameScenarios["intro"].choices);
    },
    observe_villager: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay(`${gameState.pendingNewVillager.name}의 정착을 지켜보기로 했습니다. (-5 공동체 정신)`);
    ...
    reject_villager: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay(`${gameState.pendingNewVillager.name}의 정착을 거절했습니다. (-10 공동체 정신)`);
    ...
    hold_festival: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.food >= 50 && gameState.resources.wood >= 20) { updateGameDisplay("성대한 축제를 개최했습니다! (+30 행복도, +30 공동체 정신, -50 식량, -20 나무)"); updateState({ happiness: gameState.happiness + 30, communitySpirit: gameState.communitySpirit + 30, resources: { ...gameState.resources, food: gameState.resources.food - 50, wood: gameState.resources.wood - 20 } }); }
        else { updateGameDisplay("자원이 부족하여 축제를 개최할 수 없었습니다. (-10 행복도)"); updateState({ happiness: gameState.happiness - 10 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    postpone_festival: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("축제를 다음으로 미루기로 했습니다. (-5 행복도)");
        updateState({ happiness: gameState.happiness - 5 });
        renderChoices(gameScenarios["intro"].choices);
    },
    request_donations: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (currentRandFn() < 0.6) { updateGameDisplay("주민들이 자발적으로 자원을 기부했습니다! (+20 식량, +10 나무, +10 공동체 정신)"); updateState({ resources: { ...gameState.resources, food: gameState.resources.food + 20, wood: gameState.resources.wood + 10 }, communitySpirit: gameState.communitySpirit + 10 }); }
        else { updateGameDisplay("주민들이 자원 기부에 소극적입니다. (-5 공동체 정신)"); updateState({ communitySpirit: gameState.communitySpirit - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    investigate_rumor: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (currentRandFn() < 0.7) { updateGameDisplay("소문의 근원지를 찾아 진실을 밝혀냈습니다! (+15 공동체 정신, +10 행복도)"); updateState({ communitySpirit: gameState.communitySpirit + 15, happiness: gameState.happiness + 10 }); }
        else { updateGameDisplay("소문의 근원지를 찾지 못했습니다. (-10 공동체 정신)"); updateState({ communitySpirit: gameState.communitySpirit - 10 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    calm_villagers: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("주민들에게 소문에 흔들리지 말라고 당부했습니다. (+5 공감)");
        updateState({ empathy: gameState.empathy + 5 });
        renderChoices(gameScenarios["intro"].choices);
    },
    ignore_rumor: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("소문을 무시했습니다. 소문은 더욱 확산되어 마을의 분위기를 해치고 있습니다. (-20 공동체 정신, -15 행복도)");
        updateState({ communitySpirit: gameState.communitySpirit - 20, happiness: gameState.happiness - 15 });
        renderChoices(gameScenarios["intro"].choices);
    },
    repair_together: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("주민들과 함께 피해 복구에 힘썼습니다. (+20 공동체 정신, +15 행복도, -5 나무)");
        updateState({ communitySpirit: gameState.communitySpirit + 20, happiness: gameState.happiness + 15, resources: { ...gameState.resources, wood: gameState.resources.wood - 5 } });
        renderChoices(gameScenarios["intro"].choices);
    },
    seek_external_help: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (currentRandFn() < 0.4) { updateGameDisplay("외부에서 도움을 요청하여 피해 복구에 필요한 자원을 지원받았습니다. (+10 식량, +10 나무)"); updateState({ resources: { ...gameState.resources, food: gameState.resources.food + 10, wood: gameState.resources.wood + 10 } }); }
        else { updateGameDisplay("외부의 도움을 받지 못했습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    comfort_victims: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        updateGameDisplay("피해를 입은 주민들을 위로하고 격려했습니다. (+15 공감, +10 행복도)");
    ...
    build_food_storage: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.food >= 50 && gameState.resources.wood >= 20) { updateGameDisplay("공동 식량 창고를 건설했습니다! (+10 공동체 정신, -50 식량, -20 나무)"); updateState({ communitySpirit: gameState.communitySpirit + 10, resources: { ...gameState.resources, food: gameState.resources.food - 50, wood: gameState.resources.wood - 20 } }); gameState.villages.foodStorageBuilt = true; }
        else { updateGameDisplay("자원이 부족하여 공동 식량 창고를 건설할 수 없습니다."); }
        renderChoices(gameScenarios["intro"].choices);
    },
    build_workshop: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.wood >= 30 && gameState.resources.stone >= 30) { updateGameDisplay("공동 작업장을 건설했습니다! (+10 행복도, -30 나무, -30 돌)"); updateState({ happiness: gameState.happiness + 10, resources: { ...gameState.resources, wood: gameState.resources.wood - 30, stone: gameState.resources.stone - 30 } }); gameState.villages.workshopBuilt = true; }
        else { updateGameDisplay("자원이 부족하여 공동 작업장을 건설할 수 없습니다."); }
        renderChoices(gameScenarios["intro"].choices);
    },
    build_town_hall: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.food >= 100 && gameState.resources.wood >= 50 && gameState.resources.stone >= 50) { updateGameDisplay("마을 회관을 건설했습니다! (+20 공동체 정신, +20 행복도, -100 식량, -50 나무, -50 돌)"); updateState({ communitySpirit: gameState.communitySpirit + 20, happiness: gameState.happiness + 20, resources: { ...gameState.resources, food: gameState.resources.food - 100, wood: gameState.resources.wood - 50, stone: gameState.resources.stone - 50 } }); gameState.villages.townHallBuilt = true; }
        else { updateGameDisplay("자원이 부족하여 마을 회관을 건설할 수 없습니다."); }
        renderChoices(gameScenarios["intro"].choices);
    },
    build_library: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.wood >= 80 && gameState.resources.stone >= 40) { updateGameDisplay("도서관을 건설했습니다! (+15 공감, +10 공동체 정신, -80 나무, -40 돌)"); updateState({ empathy: gameState.empathy + 15, communitySpirit: gameState.communitySpirit + 10, resources: { ...gameState.resources, wood: gameState.resources.wood - 80, stone: gameState.resources.stone - 40 } }); gameState.villages.libraryBuilt = true; }
        else { updateGameDisplay("자원이 부족하여 도서관을 건설할 수 없습니다."); }
        renderChoices(gameScenarios["intro"].choices);
    },
    ...
    puzzle_reinforce_houses: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.wood >= 10) { updateGameDisplay("목재를 사용해 주민들의 집을 보강했습니다. (+10 행복도, +5 공동체 정신, -10 나무)"); updateState({ happiness: gameState.happiness + 10, communitySpirit: gameState.communitySpirit + 5, resources: { ...gameState.resources, wood: gameState.resources.wood - 10 } }); }
        else { updateGameDisplay("목재가 부족하여 집을 보강할 수 없습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    puzzle_repair_facilities: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.wood >= 10) { updateGameDisplay("목재를 사용해 마을의 공공 시설을 보수했습니다. (+15 공동체 정신, -10 나무)"); updateState({ communitySpirit: gameState.communitySpirit + 15, resources: { ...gameState.resources, wood: gameState.resources.wood - 10 } }); }
        else { updateGameDisplay("목재가 부족하여 공공 시설을 보수할 수 없습니다. (-5 공동체 정신)"); updateState({ communitySpirit: gameState.communitySpirit - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
    puzzle_stockpile_firewood: () => {
        if (gameState.actionPoints-- <= 0) { updateGameDisplay("행동 포인트가 부족합니다."); return; }
        if (gameState.resources.wood >= 10) { updateGameDisplay("목재를 땔감으로 비축하여 주민들에게 나누어 주었습니다. (+15 행복도, -10 나무)"); updateState({ happiness: gameState.happiness + 15, resources: { ...gameState.resources, wood: gameState.resources.wood - 10 } }); }
        else { updateGameDisplay("목재가 부족하여 땔감을 비축할 수 없습니다. (-5 행복도)"); updateState({ happiness: gameState.happiness - 5 }); }
        renderChoices(gameScenarios["intro"].choices);
    },
...
function renderStats() {
    ...
    const villagerListHtml = gameState.villagers.map(v => `<li>${v.name} (${v.personality}, ${v.skill}) - 신뢰도: ${v.trust}</li>`).join('');
...
function renderChoices(choices) {
    ...
    document.getElementById('gameChoices').innerHTML = dynamicChoices.map(choice => {
...
function processDailyEvents() {
    ...
    let dailyMessage = skillBonusMessage;
    ...
    const rand = currentRandFn();
    let eventId = "intro";
    if (rand < 0.2 && gameState.villagers.length >= 2) { eventId = "daily_event_conflict"; }
    else if (rand < 0.4 && gameState.resources.food < 15) { eventId = "daily_event_resource_shortage"; }
    else if (rand < 0.6 && gameState.day % 5 === 0 && gameState.villagers.length < gameState.maxVillagers) {
    ...
    else if (rand < 0.7 && gameState.day % 7 === 0) { eventId = "daily_event_festival_request"; }
    else if (rand < 0.8 && gameState.communitySpirit < 60) { eventId = "daily_event_rumor"; }
    else if (rand < 0.9 && gameState.day > 5) { eventId = "daily_event_natural_disaster"; }
    else if (rand < 0.94 && gameState.day > 3 && gameState.resources.wood >= 10) { eventId = "daily_mini_game_resource_puzzle"; }
    else if (rand < 0.98) { eventId = "daily_mini_game_emotion_match"; }
...
function checkGameOver() {
    ...
    else if (gameState.resources.food < -5) { showFeedback(false, gameScenarios["game_over_resources"].feedback); gameEnded = true; }
